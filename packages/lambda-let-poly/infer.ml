...
let rec typ_of_typ_sig = function
  ...
  | TSTuple typs ->
      List.map typ_of_typ_sig typs |> result_all_map (fun a -> TTuple a)
  | TS "Int" -> Ok TInt
  | TS "Bool" -> Ok TBool
  | TS _ -> Error "unknown"
...
let rec substitute_single subst = function
  ...
  | TBool -> TBool
  | TArrow (t1, t2) ->
      TArrow (substitute_single subst t1, substitute_single subst t2)
  | TTuple ts -> TTuple (List.map (substitute_single subst) ts)

let substitute subst constrs =
  List.map
    (fun (t1, t2) -> (substitute_single subst t1, substitute_single subst t2))
    constrs
  ...
  | (t1, t2) :: rest -> (
      ...
      | TVar p, t | t, TVar p ->
          if occurs p t then
            ...
            let subst' =
              List.map (fun (x, t) -> (x, substitute_single rest_subst t)) subst
            in
            Ok (subst' @ rest_subst)
      | TArrow (a1, r1), TArrow (a2, r2) -> unify ((a1, a2) :: (r1, r2) :: rest)
      | TTuple ts1, TTuple ts2 ->
          if List.length ts1 <> List.length ts2 then Error "Tuple size mismatch"
          else
...
let rec constraints ctx = function
  ...
  | Lambda (param_name, type_sig, body) ->
      let param = fresh () in
      let* param_sig_constr =
        match type_sig with
        | Some x ->
            let* typ = typ_of_typ_sig x in
            Ok [ (param, typ) ]
        | None -> Ok []
      in
  ...
  | If (cond, yes, no) ->
      ...
      Ok
        ( no_typ,
          cond_constr @ yes_constr @ no_constr
          @ [ cond_bool_constr; branch_constr ] )
  | Int _ -> Ok (TInt, [])
  | Bool _ -> Ok (TBool, [])
  | Tuple exprs ->
